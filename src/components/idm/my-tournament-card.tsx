      ...
      <CardContent className="p-4 relative z-10">
        <div className="flex items-center gap-3 mb-3">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${dt.iconBg}`}>
            <Target className={`w-4 h-4 ${dt.neonText}`} />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-bold text-gradient-fury">Cari Turnamen Kamu</h3>
            <p className="text-[9px] text-muted-foreground">Masukkan nama atau gamertag</p>
          </div>
          {submittedName && (
            <Button size="sm" variant="ghost" className="h-6 text-[9px] shrink-0" onClick={() => { setSubmittedName(''); setSearchName(''); }}>
              ← Kembali
            </Button>
          )}
        </div>
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-8 h-8 text-xs glass"
              maxLength={30}
            />
          </div>
          <Button
